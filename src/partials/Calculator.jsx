...
const Calculator = () => {
  ...
  const [medianSurvivalWithTreatment, setMedianSurvivalWithTreatment] = useState(null);
  const [meanSurvivalWithTreatment, setMeanSurvivalWithTreatment] = useState(null);
  ...
  useEffect(() => {
    const calculateResults = () => {
      ...
      if (time > 0 && milestoneProbability > 0 && hazardRatio > 0) {
        ...
        const calculatedValue = (1 / Math.exp((lnValue / time) * hazardRatio * time)) * 100;
        const medianSurvivalValue = Math.log(2) / (Math.log(1 / (milestoneProbability / 100)) / time);
        const meanSurvivalValue = (1.44 * medianSurvivalValue);
        const medianSurvivalWithTreatmentValue = (Math.log(2) / ((Math.log(1 / (milestoneProbability / 100)) / time) * hazardRatio)) 
        const meanSurvivalWithTreatmentValue = (1.44 * medianSurvivalWithTreatmentValue);
      ...
      <div className='overflow-x-auto flex flex-col lg:flex-row justify-between'>
        <div className=''>
          ...
          <table className='mt-1 '>
            <tbody className=''>
              <tr>
                <td className='text-left font-medium  border p-2'>Approximated Milestone Probability:</td>
                <td className='text-right p-2 border'>{result.toFixed(3)}%</td>
              </tr>
              <tr>
                <td className='text-left font-medium border p-2'>
                  <p>Approximated absolute risk reduction (ARR):</p>
                  <p className='text-xs italic'>At time t with treatment vs control based on HR point estimate</p>
                </td>
                <td className='text-right p-2 border'>{(result - milestoneProbability).toFixed(3)}%</td>
              </tr>
        ...
        <div className='pt-2 lg:mt-0'>
          ...
          <table className='mt-1'>
            <tbody>
            <tr>
                    <td className='text-left font-medium border p-2'>Median Survival with Control:</td>
                    <td className='text-right p-2 border'>{medianSurvival?.toFixed(2)} <span className='text-xs'>({survivalUnit})</span></td>
                  </tr>
                  <tr>
                    <td className='text-left font-medium border p-2'>Mean Survival with Control:</td>
                    <td className='text-right p-2 border'>{meanSurvival?.toFixed(2)} <span className='text-xs'>({survivalUnit})</span></td>
                  </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className='pt-2 lg:mt-0'>
          ...
          <table className='mt-1'>
            <tbody>
            <tr>
                    <td className='text-left font-medium border p-2'>Median Survival with Treatment:</td>
                    <td className='text-right p-2 border'>{medianSurvivalWithTreatment?.toFixed(2)} <span className='text-xs'>({survivalUnit})</span> </td>
                  </tr>
                  <tr>
                    <td className='text-left font-medium border p-2'>Mean Survival with Treatment:</td>
                    <td className='text-right p-2 border'>{meanSurvivalWithTreatment?.toFixed(2)} <span className='text-xs'>({survivalUnit})</span></td>
                  </tr>
            </tbody>
          </table>
          </div>
